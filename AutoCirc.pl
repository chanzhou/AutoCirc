...
Examples :
	perl $0  -g /human_genome/hg19.fa -I /bowtie2_index/hg19 --bam unmapped.bam -b /hg19/Annotation/Refseq.gene.strandarded.bed --mis 0 --min 100 --max 100000 -o autocirc_output 
	or
	perl $0  -g /human_genome/hg19.fa -I /bowtie2_index/hg19 --bam unmapped.bam -b /hg19/Annotation/Refseq.gene.strandarded.bed
	or
	perl $0  -g /human_genome/hg19.fa --bam unmapped.bam -o autocirc_output (if do not have the reference gene annotations)   
...
if ( (not $unmapped_bam) or (not $genome_index) or (not $ref_genome)) {
...
if ((($ref_gene_bed ne "NA") and (not $ref_gene_bed =~/\S+\.bed/) ) or 
    ((not $ref_gene_bed =~/^\/\S+/) and (not $ref_gene_bed =~/^\~\/\S+/)) ){
	main::HELP_MESSAGE;
} 
...
warn "../script/unmapped2anchors.pl $unmapped_sam > unmapped_anchors.qfa\n";
my $generate_anchors=`../script/unmapped2anchors.pl $unmapped_sam > unmapped_anchors.qfa`;
...
my $map_anchors=`bowtie2 -p4 --reorder --mm -D20 --score-min=C,-15,0 -q -x $genome_index -U unmapped_anchors.qfa 1>bowtie2.anchors.out.sam 2>bowtie2.anchors.log`;
warn "Now mapping the anchors into reference genome\n";
...
warn "../script/autocirc_gtag.v1.0 bowtie2.anchors.out.sam  $ref_genome $mismatch $min $max circ.gtag.bed linear.gtag.bed backsplice.raw.bed 2> clog.AutoCirc_gtag\n";
my $generate_gtagcirc=`../script/autocirc_gtag.v1.0 bowtie2.anchors.out.sam  $ref_genome $mismatch $min $max circ.gtag.bed linear.gtag.bed backsplice.raw.bed 2> clog.AutoCirc_gtag`;
...
if($ref_gene_bed=~/\S+\.bed/){
	...
	warn "intersectBed -a backsplice.raw.bed -b $ref_gene_bed -f 1 -wa -wb | cut -f 1-9,13,15,19-22 > backwards.annotatedexons.bed\n";
	
	my $generate_circ_annotatedexons=`intersectBed -a backsplice.raw.bed -b $ref_gene_bed -f 1 -wa -wb | cut -f 1-9,13,15,19-22 > backwards.annotatedexons.bed`;
	until($generate_circ_annotatedexons ne 0){
		print ".........\n";
		sleep(10);
	}

	warn "../script/filter_exon_boundaries.pl backwards.annotatedexons.bed 10 > circ.exonboundaries.unstrand.bed\n";
	my $filter_exonboundaries=`../script/filter_exon_boundaries.pl backwards.annotatedexons.bed 10 > circ.exonboundaries.unstrand.bed`;
	...
	warn "Now excluding the pseudo circRNAs which across two duplicated genes\n";
	...
	my $rem_2=`../script/circ_across2genes.pl circ.ref_gene.bed > circ.2genes.bed`;
	until($rem_2 ne 0){
		print ".........\n";
		sleep(3);
	}

	my $rem_3=`../script/circ2mboundaries.pl  circ.2genes.bed > circ.2genes.eboundaries.bed 2>err`;
	until($rem_3 ne 0){
		print ".........\n";
		sleep(3);
	}

	my $rem_4=`../script/my_join.pl -a circ.combined.bed -b circ.2genes.eboundaries.bed -F 4 -f 4 | awk '\$9==\"\"' | cut -f 1-8 > circ.final.bed`;
	until($rem_4 ne 0){
		print ".........\n";
		sleep(3);
	}
	
	system "rm backwards.annotatedexons.bed";
	system "rm circ.2genes.bed";
	system "rm circ.2genes.eboundaries.bed";
	system "rm circ.combined.bed";
	system "rm circ.exonboundaries.unstrand.bed";
